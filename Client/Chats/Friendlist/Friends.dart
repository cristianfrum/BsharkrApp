...
class Friends extends StatefulWidget {
  ...
  Friends({this.clientUser, this.newFriend, this.parent, this.index});
  @override
  State createState() => FriendsState();
}
...
class FriendsState extends State<Friends>
  ...
  Widget buildItem(BuildContext context, DocumentSnapshot document, int index) {
    ...
    if (_trainerUser.id == prefs.getString('id')) {
      ...
      bool dontShowRequestButton = false;
      _trainerUser.clients.forEach((element) {
        if (element.clientAccepted == false &&
            element.clientId == prefs.getString('id')) {
          dontShowRequestButton = true;
        }
      });
      if (dontShowRequestButton == false) {
        if (disabledIndexes.contains(_trainerUser.id) == true) {
          dontShowRequestButton = true;
        }
      }
      return Container(
                      ...
                      child: IconSlideActionModifiedRight(
                        ...
                        onTap: () async {
                          ...
                          batch.updateData(
                            ...
                            {
                              'trainersMap.${_trainerUser.id}': false,
                            },
                          );
                          batch.updateData(
                            db
                                ...
                                .document(_trainerUser.id),
                            {
                              ...
                              'businessRequestDate.${prefs.getString('id')}':
                                  DateTime.now()
                            },
                          );

                          QuerySnapshot query3 = await Firestore.instance
                              .collection(
                                  'trainerCollaborationRequestsReceived')
                          ...
                          if (mounted) {
                            setState(() {
                              disabledIndexes.add(_trainerUser.id);
                            });
                          }
                        },
                      ),
                  ...
                  onTap: () {
                    Navigator.push(
                        ...
                        DeletePermissionPopupProfile(
                            trainer: _trainerUser, friendOrbusiness: false));
                  },
                ),
              ),
            ],
  ...
  void initState() {
    ...
    super.initState();
    _getTrainers();
    widget.clientUser.friends.forEach((element) {
  ...
  _getTrainers() async {
    ...
    if (querySnapshot.documents.length != 0) {
      ...
      for (int i = 0; i < _trainers.length - 1; i++) {
        for (int j = i + 1; j < _trainers.length; j++) {
          if (TrainerUser(_trainers[i]).rating <
              TrainerUser(_trainers[j]).rating) {
            var aux = _trainers[i];
            _trainers[i] = _trainers[j];
            _trainers[j] = aux;
          }
        }
      }
  ...
  check() async {
    ...
    if (realNumber > verifyIds.length) {
      widget.clientUser.friends.forEach((element) async {
        bool flag = false;
        if (element.friendAccepted == true) {
          ...
          if (flag == false) {
            ...
            if (finalIndex != -1) {
              ...
              _trainers.forEach((elementx) {
                if (TrainerUser(elementx).id == element.friendId) {
                  ultimulFlag = true;
                }
              });
  ...
  build(BuildContext context) {
    if (widget.clientUser.newFriend == true && widget.index == 1) {
      Firestore.instance
          .collection('clientUsers')
          .document(prefs.getString('id'))
          .updateData(
        {'newFriend': FieldValue.delete()},
      );
    }
    check();
    return (_trainers.length == 0 && _loadingTrainers == true)
        ...
        : _trainers.length != 0
            ? Scaffold(
                backgroundColor: backgroundColor,
                body: _loadingTrainers == false
                    ? ListView.builder(
                        controller: _scrollController,
                        itemBuilder: (context, index) =>
                            buildItem(context, _trainers[index], index),
