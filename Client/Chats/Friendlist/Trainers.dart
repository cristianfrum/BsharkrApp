...
class Trainers extends StatefulWidget {
  ...
  Trainers({this.clientUser, this.newTrainer, this.parent, this.index});
  @override
  State createState() => TrainersState();
}

class TrainersState extends State<Trainers>
    with AutomaticKeepAliveClientMixin<Trainers> {
  ...
  Widget buildItem(BuildContext context, DocumentSnapshot document) {
    ...
    bool seenFlag = false;
    widget.clientUser.unseenMessagesCounter.forEach((user) {
      if (user.userId == _trainerUser.id) {
        seenFlag = true;
      }
    });
      ...
      _trainerUser.specializations.forEach((special) {
        if (special.certified == true) {
          if (special.specialization == 'kangooJumps') {
            trainersSpecializations =
                trainersSpecializations + "Kangoo Jumps, ";
          } else {
                  ...
                  onTap: () {
                    Navigator.push(
                        ...
                        DeletePermissionPopup(
                            trainerUser: _trainerUser, parent: this));
                  },
                ),
              ),
            ],
                    ...
                    Container(
                      ...
                      child: Row(
                        children: <Widget>[
                          ...
                          Container(
                            width: 240 * prefs.getDouble('width'),
                            child: Row(
  ...
  void initState() {
    ...
    super.initState();
    _getTrainers();
    widget.clientUser.trainers.forEach((element) {
      if (element.trainerAccepted == true) {
        verifyIds.add(element.trainerId);
      }
    });
  }
  ...
  _getTrainers() async {
    ...
    Query q = Firestore.instance
        ...
        .where('trainerMap.${prefs.getString('id')}', isEqualTo: true);
  ...
  check() async {
    ...
    if (realNumber > verifyIds.length) {
      widget.clientUser.trainers.forEach((element) async {
        bool flag = false;
        if (element.trainerAccepted == true) {
          verifyIds.forEach((element1) {
            if (element1 == element.trainerId) {
              flag = true;
            }
          });
          if (flag == false) {
            QuerySnapshot query = await Firestore.instance
                .collection('clientUsers')
                .where('id', isEqualTo: element.trainerId)
                .getDocuments();
            bool flaggy = false;
            int index = 0;
            int finalIndex = -1;
            _trainers.forEach((element2) {
              if (TrainerUser(query.documents[0]).rating >=
                      TrainerUser(element2).rating &&
                  flaggy == false) {
                flaggy = true;
                finalIndex = index;
              }
            ...
            if (flaggy == false && _trainers.length < 6) {
              finalIndex = index;
            }
            if (finalIndex != -1) {
              bool ultimulFlag = false;
              _trainers.forEach((elementx) {
                if (TrainerUser(elementx).id == element.trainerId) {
                  ultimulFlag = true;
                }
              });
              if (ultimulFlag == false) {
                if (mounted) {
                  setState(() {
                    _trainers.insert(finalIndex, query.documents[0]);
                    verifyIds.add(element.trainerId);
                  });
                }
              }
    ...
    if (realNumber < verifyIds.length) {
      verifyIds.forEach((element1) async {
        ...
        if (flag == false) {
          if (_trainers
                  .where((element) => TrainerUser(element).id == element1)
                  .toList()[0] !=
              null) {
            bigBool = true;
            auuxx.add(element1);
  ...
  Widget build(BuildContext context) {
    if (widget.clientUser.newBusiness == true && widget.index == 2) {
      Firestore.instance
          .collection('clientUsers')
          .document(prefs.getString('id'))
          .updateData(
        {'newBusiness': FieldValue.delete()},
      );
    }
    check();
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    ...
                    Container(
                      ...
                      child: Text(
                        "Start looking for trainers right now.",
                        style: TextStyle(
                            letterSpacing: 0.75,
...
class DeletePermission extends StatefulWidget {
  ...
  final TrainersState parent;
  DeletePermission({Key key, @required this.trainerUser, this.parent})
      : super(key: key);
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              Padding(
                ...
                child: Text(
                  "Are you sure you want to delete your partnership with ${trainerUser.firstName} ${trainerUser.lastName}? Please take in consideration that the friendship will be over as well.",
                  style: TextStyle(
                    fontFamily: 'Roboto',
                      ...
                      onPressed: () async {
                        ...
                        batch.updateData(
                          db
                              ...
                              .document(prefs.getString('id')),
                          {
                            'trainersMap.${trainerUser.id}':
                                FieldValue.delete(),
                            'friendsMap.${trainerUser.id}': FieldValue.delete()
                          },
                        );
                        batch.updateData(
                        ...
                        if (mounted) {
                          widget.parent.setState(() {
                            widget.parent._trainers.remove(widget
                                .parent._trainers
                                .where((element) =>
                                    TrainerUser(element).id == trainerUser.id)
                                .toList()[0]);
                            widget.parent.verifyIds.remove(trainerUser.id);
                          });
                        }
                      },
                      child: Text(
                                                                      ...
                                                                      if (query.documents.length !=
                                                                        ...
                                                                        if (finalIndex !=
                                                                          ...
                                                                          widget
                                                                              .parent
                                                                              ._trainers
                                                                              .removeWhere((element) => TrainerUser(element).id == widget.bookedTrainerId);
                                                                          widget
                                                                              .parent
                                                                              ._trainers
                                                                              .insert(finalIndex, query.documents[0]);
                                                                        }
...
class Vote extends PopupRoute<void> {
  ...
  final TrainersState mainParent;
  final _TrainerProfilePageState parent;
  final TrainerUser trainer;
  final ClientUser clientUser;
  @override
                                          ...
                                          widget.mainParent._trainers
                                              .removeWhere((element) =>
                                                  ...
                                                  TrainerUser(
                                                          query.documents[0])
                                                      .id);
                                          widget.mainParent._trainers.insert(
                                              finalIndex, query.documents[0]);
                                        });
                                      }
                                    }
...
class TrainerPublicClasses extends StatefulWidget {
  ...
  TrainerPublicClasses(
      {@required this.imTrainer, this.imClient, this.parent, this.mainParent});
  @override
...
class PopUpVotedRoute extends PopupRoute<void> {
  @override
  Color get barrierColor => Color.fromARGB(255, 8, 8, 13).withOpacity(0.98);
...
class PopUpVoted extends StatefulWidget {
  @override
  State createState() => PopUpVotedState();
}

class PopUpVotedState extends State<PopUpVoted> {
  @override
  Widget build(BuildContext context) {
    return Center(
      child: Dialog(
